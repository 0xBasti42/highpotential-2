...
import { CurrencySettler } from "@base/utils/CurrencySettler.sol";
...
contract LimitOrderHook is BaseHook, IUnlockCallback {
    ...
    function _placeOrder(
        ...
        unchecked {
            ...
            orderInfo.liquidity[sender] += liquidity;
        }
        // set the currency checkpoints for the sender. These amounts are stored so that the user cannot steal
        ...
        orderInfo.checkpoints[sender].amountCurrency0 = orderInfo.currency0Total;
        orderInfo.checkpoints[sender].amountCurrency1 = orderInfo.currency1Total;
        ...
        (uint256 amount0Fee, uint256 amount1Fee) = abi.decode(
            poolManager()
                .unlock(
                    abi.encode(
                        CallbackData(
                            CallbackType.Place, abi.encode(PlaceCallbackData(key, sender, zeroForOne, tick, liquidity))
                        )
                    )
                ),
            (uint256, uint256)
        ...
        emit Place(sender, orderId, key, tick, zeroForOne, liquidity);
    }
     ...
     * removed liquidity. Note that partial cancellation is not supported - the entire liquidity added by the sender will be removed.
     * Note also that cancelling an order will cancel the order placed by the sender, not orders placed by other users in the same tick range.
    ...
    // get the order
        ...
        // get the liquidity added by the sender
        uint128 liquidity = orderInfo.liquidity[sender];

        // revert if the liquidity is 0
        if (liquidity == 0) revert ZeroLiquidity();

        // delete the liquidity from the order
        delete orderInfo.liquidity[sender];
        ...
        emit Cancel(sender, orderId, key, tickLower, zeroForOne, liquidity);
    }
    ...
    // get the order info
        ...
        uint256 checkpointAmountCurrency0 = orderInfo.checkpoints[sender].amountCurrency0;
        uint256 checkpointAmountCurrency1 = orderInfo.checkpoints[sender].amountCurrency1;

        // calculate the amount of currency0 and currency1 owed to the sender
        ...
        emit Withdraw(sender, orderId, liquidity);
    }
    ...
    ) public virtual onlyPoolManager returns (bytes memory returnData) {
    ...
    function _handlePlaceCallback(
        ...
        (BalanceDelta principalDelta, BalanceDelta feesAccrued) = manager.modifyLiquidity(
            key,
        ...
        if (feesAccrued.amount0() > 0) {
            key.currency0.take(manager, address(this), amount0Fee = uint256(uint128(feesAccrued.amount0())), true);
        }
        if (feesAccrued.amount1() > 0) {
            key.currency1.take(manager, address(this), amount1Fee = uint256(uint128(feesAccrued.amount1())), true);
        }
        ...
        if (delta.amount0() < 0) {
            ...
            key.currency0.settle(manager, placeData.owner, uint256(uint128(-delta.amount0())), false);
        } else {
            ...
            key.currency1.settle(manager, placeData.owner, uint256(uint128(-delta.amount1())), false);
        }
    }
    ...
    function _handleCancelCallback(
        ...
        (BalanceDelta cancelDelta, BalanceDelta feesAccrued) = manager.modifyLiquidity(
            cancelData.key,
            ModifyLiquidityParams({
        ...
        if (principalDelta.amount0() > 0) {
            cancelData.key.currency0.take(manager, cancelData.to, uint256(uint128(principalDelta.amount0())), false);
        }
        ...
        if (principalDelta.amount1() > 0) {
            cancelData.key.currency1.take(manager, cancelData.to, uint256(uint128(principalDelta.amount1())), false);
        }
    }
    ...
    function _handleWithdrawCallback(
        ...
        if (withdrawData.currency0Amount > 0) {
            ...
            manager.burn(address(this), withdrawData.currency0.toId(), withdrawData.currency0Amount);
            // take the currency0 from the pool and send it to the `to` address
            manager.take(withdrawData.currency0, withdrawData.to, withdrawData.currency0Amount);
        }
        ...
        if (withdrawData.currency1Amount > 0) {
            ...
            manager.burn(address(this), withdrawData.currency1.toId(), withdrawData.currency1Amount);
            // take the currency1 from the pool and send it to the `to` address
            manager.take(withdrawData.currency1, withdrawData.to, withdrawData.currency1Amount);
        }
    }
